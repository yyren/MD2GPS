...
parser = argparse.ArgumentParser(description='The script of KnowledgeAgent prediction.')
...
print("KnowledgeAgent analysis starts!")
...
if os.path.exists(input_file):
    ...
    raise FileNotFoundError(f"The KnowledgeAgent ranking result file is not exist: {input_file}")

# If there is no gene in PDS result, diagnosis end.
if gene_num == 0:
    KnowledgeAgent_pathogenic_gene_order = "Not Found"
    with open(output_file, 'w', encoding='utf-8') as file:
        file.write(f"The patient has no gene list from KnowledgeAgent rank result, diagnosis end.")
elif gene_num > 0:
    ...
    # print(KnowledgeAgent_prompt_fill)
    
    # get the KnowledgeAgent response
    for attempt in range(max_attempts):
        KnowledgeAgent_analysis = LLM_diagnose_pathogenic_gene_order(KnowledgeAgent_prompt_fill)  # KnowledgeAgent response
